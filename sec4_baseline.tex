...
\begin{theorem}
\label{THM:RP_ETA}
Let $\eta = (\lceil \frac{K}{L} \rceil - 1)*(G-1)+K+L-1$. Then it guarantees 
the completeness of the partition. Further:
let $\eta^*$ be the optimal value for a given $G,L,K$, then the following holds:
$\eta \leq \eta^* + L-1$.
\end{theorem}
\begin{proof}
See Appendix.
