...
to discover co-moving objects. A \emph{co-movement} pattern~\cite{li2013managing} 
...
\begin{figure}[h]
\centering
\includegraphics[width=0.4\textwidth]{related_work.eps}
...
\hline 
flock~\cite{gudmundsson2004flock} & disk-based &  global & {\tiny $O(|\mathbb{O}||\mathbb{T}|(M + \log(|\mathbb{O}|)))$} \\ 
\hline 
...
\hline 
\end{tabular} 
\caption{Constraints and complexities of co-movement patterns. The time complexity indicates the performance in the worst case, where $|\mathbb{O}|$ is the total number of objects, $|\mathbb{T}|$ is the number of descritized timestamps and $M$ is the minimum cluster cardinality. }
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.23\textwidth}
		...
		\subcaption{$1k$ timestamps vary $O$} 
    \label{fig:fig1}
    \end{subfigure}
    \begin{subfigure}[b]{0.23\textwidth}
            \centering
            \includegraphics[width=\textwidth]{rw_perf_T.eps}
         \subcaption{$1k$ objects vary $T$}
    \label{fig:fig2}
    \end{subfigure}
    \vspace{-2mm}
...
SPARE runs in 12 minutes using 162 cores, whereas
centralized solutions take from 10 to 30 hours.
