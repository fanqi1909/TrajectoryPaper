...
It is notable that \emph{platoon} pattern is able to represent other
patterns by setting appropriate parameters. However, we observe
that such a pattern definition, due to its overly-relaxed temporal constraints,
loses the fine control of a pattern
which leads to noisy results and unnecessary computations. We name this
scenario as \emph{loose-connection} anomaly. To illustrate, as shown in Figure~\ref{fig:platoon_weakpoint}, the two objects $o_1, o_2$ form a \emph{platoon} pattern 
$\{o_1,o_2:1,2,3,102,103,104\}$. However, the consecutive segments are $98$ timestamps apart, 
making the co-moving behavior very weak. In reality, 
such an anomaly is likely induced by the periodic movements of unrelated objects 
such as, vehicles stopping at the same petrol station, animals pausing at the same water source etc. 
In fact, none of the temporal-relaxed patterns (e.g., \emph{swarm}, \emph{group} and \emph{platoon}) 
are able to directly avoid such an anomaly.
...
\caption{\emph{Loose-connection} anomaly. The consecutive segment of $o_1$ and $o_2$ are 98 timestamps apart, however, the pattern $\{o_1, o_2: 1,2,3,102,103,104\}$ is valid in \emph{platoon}, \emph{swarm} and \emph{group} results.}
...
built on top of centralized indexes which face scalability challenges. 
To the best of our knowledge, the maximum number of trajectories
ever evaluated is up to a million data points.
In practice, it is rather common to collect hundreds of millions data points
and the scalabilities of current solutions are left unknown. 
We conduct a theoretical analysis on the worst-case complexity 
(as listed in Table~\ref{tbl:existing_co_patterns}) as well as 
an experimental evaluation with a real trajectory database 
with upto 2.4 million points (as shown in Figure~\ref{fig:related_work_scalability}). 
Results show that their performances degrade dramatically as the dataset scales up. For instances,
the performance of \emph{swarm} drops five times as the number of objects grows from \emph{1k} to \emph{2k}. Similarly,
the performance of \emph{group} drops over seven times as the number of snapshots grows from \emph{1.4k} to \emph{2.4k}.
Therefore, directly applying these patterns on billion-scale real trajectories is impractical.
