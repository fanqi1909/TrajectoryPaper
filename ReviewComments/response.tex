...
%we found that Spark-SQL Catalyst optimizer assign the window query using only one
%executor. It is because that our TRPM-SQL query does not have a ``partition by'' clause, and
%the Catalyst optimizer treats the entire dataset as a SINGLE partition. Such a plan
%surrenders all the parallel benefits of the Spark system. 
% 
%
%
