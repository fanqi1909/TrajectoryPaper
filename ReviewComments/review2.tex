...
mining Gap-restricted Sequential Patterns (GSP). However, the novelty
of this paper is not merely on introducing the gap-constraint to the
co-movement pattern. Instead, we provide a one-stop solution (both in modeling and in technical)
for mining various co-movement patterns with flexible constraints. With
these constraints, users are able to define a more accurate pattern
which are free from the anomalies (e.g., loss-connection anomaly as in Figure 2).

Besides, the nature of GSP and GCMP are different. The goal of GSP 
...
in Section 2.5.
%
%When counting the occurrence of a sequence, 
%the object the sequence belongs does not matter. 
%Therefore, a pattern in FS may corresponds to any objects where
%the closeness or distinction of objects are not statisfied. Due
%to this difference, FS solutions cannot be directly applied on GCMP.
%
%
%We agree with the reviewer that gap parameter is 
%also used in the Sequence Mining problem. However, 
%Sequence mining problem is different from the GCMP mining problem. 
%The major difference is that in sequence mining problem, the object
%itself is not considered. In particular, the frequency of a sequence is 
%counted as the occurrence of a sequence. Thus, an object may
%contribute multiple occurrence of a sequence. Conversely, GCMP
%in addition to impose the gap parameter in the temporal constraint, also 
%require the objects in the pattern to be close. Due to this difference,
%Sequence mining solutions cannot support GCMP mining.
%
%that the gap parameter is an old concept. 
%However, the novelty of GCMP lies in the unification and generalization of existing patterns which provide
%a one-stop solution for co-movement pattern detection. And the
%gap parameter merely is used for solving the loose-connection anomaly (as in Figure 2). 
%
%Moreover, the problem of Sequence mining is also different from Trajectory mining.
%The major difference is that sequence mining do not consider the distinction between
%objects and the closeness among objects. This makes the solutions in Sequence mining
%unable to be applied in GCMP. We summarize these points in Section 2.5 of the revised paper.
%
...
of SPARE, a pattern $c$ is outputted if non of its superset would become a valid pattern (Line 12-14).
This prohibits the output $c$ to be a subset of any patterns.
These mechanisms effectively condense the output size by subsuming smaller
...
the number of potential patterns.
...
However, the domain-specific constraints (such as directions in road networks)
...
pattern discovery in broader scenarios such as user check-in history in social networks, visitor movements in a building and 
